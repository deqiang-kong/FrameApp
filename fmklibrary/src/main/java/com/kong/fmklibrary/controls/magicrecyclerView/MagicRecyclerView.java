...
import com.kong.fmklibrary.R;

import java.util.ArrayList;

import static com.kong.fmklibrary.controls.magicrecyclerView.BaseRecyclerAdapter.TYPE_TAGS;
...
public class MagicRecyclerView extends RecyclerView {
    ...
    public void onScrolled(int dx, int dy) {
        ...
        if (firstItemType == TYPE_TAGS && !tags.contains(mRecyclerAdapter.getTag(firstVisibleItemPosition))) {
            tags.add(mRecyclerAdapter.getTag(firstVisibleItemPosition));
        }
        if (mOnTagChangeListener != null) {
            if (dy > 0) { //向上滑动，滑到Tag时触发
                if (firstItemType == TYPE_TAGS) { //上滑到 Item 变 Tag 时触发
                ...
                if (firstItemType != TYPE_TAGS && lastItemType == TYPE_TAGS) { //下滑到 Tag 上边沿时触发
                    if (tags.size() == 0) {
                        return;
                    }
                    if (!tagChanged) {
